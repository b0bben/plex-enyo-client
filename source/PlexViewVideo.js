...
enyo.kind({
    ...
    components: [
        ...
        { name: "video", kind: "enyo.Video", className: "video-default", showControls: false, style: "width: 600px;height:600px;"},
    ...
    pmoChanged: function() {
       if (this.pmo !== undefined) {
        ...
        if (lastPlayTimeRecorded && !isNaN(lastPlayTimeRecorded)) {
...
//console.log('****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.create(): instance='+this.instanceId+' on "'+this.dbEntry.path+'"');
    ...
    updateControlbarToLastPlayTime: function () {
        ...
        if (timePlayed && timePlayed > 0 && timePlayed < duration) {
    ...
    initVideo: function (postProcess) {
        ...
        var left = 0;
        //var node = this.video = document.createElement("video");
        //var node = document.getElementById('myHtml5Video');
...
//console.log('****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.captureDomEvent(): a mousedown on controlbar from "'+ev.target.id+'"');
    ...
    onVideoLoad: function (ev) {
        if (!this.video || !this.video) { return; }  // should never happen
        ...
        console.log('****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.onVideoLoad(): "'+this.pmo.title+'" w:'+node.videoWidth+', h:'+node.videoHeight);

        var _duration = node.duration;
        _duration = (_duration && !isNaN(_duration)) ? _duration : this.duration;
    ...
    */
    ...
    updateTimeTallies: function (newPos) {
        ...
        this.log("timeToSeek: " + timeToSeek);
    ...
    requestVideoSeek: function (newPos) {
        ...
        if (this.videoSeekingRequestPending) {
            console.log("****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.requestVideoSeek(newPos="+newPos+"): PENDING seek to: "+newPos/100*this.video.duration);
        ...
        try {
            ...
            if (node.seekable) {
                ...
                node.currentTime = Math.floor(timeSeekTo).toFixed(2);
    ...
    onVideoSeekedEventNotify: function (ev) {
        ...
        //this.log("PAUSING IN SEEKING, WILL START PLAYING IN 1SEC");
        //setTimeout(function () {thisInst.playVideo();}, 1000);
    },
    ...
    onSeekerTapped: function (inSender, pos, ev) {
        ...
        if (this.loadState == this.IS_LOADED && this.video) {
    ...
    playVideo: function (param) {
        ...
        switch (this.loadState) {
            case this.IS_LOADED:
                this.log("loadstate: " + this.loadState + "/IS_LOADED");
                this.isPlaying = true;
                this.adjustPlayStartTime();
                doVideoPlay();
                break;
            case this.IS_NOT_LOADED:
                this.log("loadstate: " + this.loadState + "/IS_NOT_LOADED");
            ...
            case this.IS_LOADEDING:
                this.log("loadstate: " + this.loadState + "/IS_LOADEDING");
                this.isAutoStart = true;
                break;
            default:
    ...
    doPause: function () {
        ...
        console.log('****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.doPause(): instanceId='+this.instanceId+' ('+this.mediadService+'), REQ to pause "'+this.pmo.title+'"...');
        this.video.pause();
        var thisInst = this;
        setTimeout(function () {
            thisInst.doPause();
...
//console.log("****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.isTailgatingEvent(): "+(now.getTime()-this.lastClickTime.getTime())+"ms from last");
    ...
    onResizeClicked: function (inSender, ev) {
        ...
        this.hideControlsWithTimeout(4000);
    },
...
//console.log('****@@@@@@><@@@@@@**** vidslide  PlexViewVideo.adjustVideoSizeDeviceMode(): on "'+this.dbEntry.path+'", videoWidth='+this.video.videoWidth+', videoHeight='+this.video.videoHeight);
        }
        var containerAr = containerWidth/containerHeight;
...
//console.log("**@@@@@**@@@@@**@@@@@** vidslide photos.PlexViewVideo.adjustVideoSizeDeviceMode: orientation:portrait, fit, "+width+"X"+height+", Topaz:"+containerWidth+"X"+containerHeight);
    ...
    requestVideoFillFitMode: function () {
        ...
        if (this.mediadService && this.isPlaying) {
            ...
            */
            this.video.setAttribute("x-palm-media-extended-fitmode",
    ...
    adjustPlayStartTime: function () {
        if (!this.video || !this.video) { return; }
        var node = this.video;
    ...
    recordVideoState: function () {
        ...
        var node;
        if (this.loadState != this.IS_LOADED || !this.video || !this.video) {
    ...
    destroy: function () {
        ...
        var parentNode;
        if (this.video) {
            parentNode = this.video.parentNode;
            if (parentNode) {
